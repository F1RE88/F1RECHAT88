...
import { Search, UserPlus, UserX, Check, X, Users, Plus } from "lucide-react";
...
export default function FriendsSidebar({
  ...
  const getProfileImageUrl = (user) => {
    if (user?.profile_image) return `${API}/files/${user.profile_image}`;
    return null;
  };
  ...
  return (
    <div className="flex flex-col h-full" data-testid="friends-sidebar">
      ...
      <div className="p-4 border-b border-neutral-800">
        {/* Tabs */}
        <div className="flex gap-1 mb-3 bg-[#121212] rounded-lg p-1">
          <button
            data-testid="tab-friends"
            onClick={() => setActiveTab("friends")}
            className={`flex-1 py-1.5 px-3 rounded-md text-xs font-semibold transition-colors font-['Cairo'] ${
              activeTab === "friends"
          ...
          <button
            ...
            className={`flex-1 py-1.5 px-3 rounded-md text-xs font-semibold transition-colors font-['Cairo'] flex items-center justify-center gap-1 ${
              activeTab === "groups"
                ? "bg-red-600 text-white"
                : "text-gray-400 hover:text-white"
            }`}
        ...
        {activeTab === "friends" && (
          <>
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs text-gray-500 font-['Tajawal']">{friends.length} friends</span>
        ...
        {activeTab === "groups" && (
          <div className="flex items-center justify-between">
            <span className="text-xs text-gray-500 font-['Tajawal']">{groups.length} groups</span>
            <button
              data-testid="create-group-trigger"
              onClick={onCreateGroup}
              className="bg-red-600/20 text-red-400 px-3 py-1 rounded-full text-xs font-semibold hover:bg-red-600/30 transition-colors flex items-center gap-1"
            >
      ...
      {/* Friend Requests */}
      {activeTab === "friends" && showRequests && friendRequests.length > 0 && (
      ...
      <div className="flex-1 overflow-y-auto p-2">
          ...
          <div data-testid="friends-list">
            {friends.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-center p-6">
                ...
                <p className="text-gray-500 text-sm font-['Tajawal']">No friends yet, it's quiet here...</p>
                <p className="text-gray-600 text-xs mt-2 font-['Tajawal']">Search for friends using @username</p>
              </div>
            ) : (
              friends.map((friend) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-colors mb-1 text-left ${
                      selectedFriend?.id === friend.id && !selectedGroup
        ...
        {activeTab === "groups" && (
          <div data-testid="groups-list">
            {groups.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-center p-6">
                <Users className="w-16 h-16 text-red-600/30 mb-3" />
                <p className="text-gray-500 text-sm font-['Tajawal']">No groups yet</p>
                <p className="text-gray-600 text-xs mt-2 font-['Tajawal']">Create a group with your friends!</p>
              </div>
            ) : (
              groups.map((group) => (
                <button
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500">
                      ...
                      {group.last_message && ` · ${group.last_message.sender_username}: ${group.last_message.content.substring(0, 20)}${group.last_message.content.length > 20 ? '...' : ''}`}
                    </p>
                  </div>
                </button>
              ))
